...
import { lectureRowScore, computeScore } from './scoringEngine';

// 1.1 per-course rules (owner decisions 2026-09-11). The frontend port has the
// same table in frontend/src/utils/lectureRowScore.test.ts — keep them equal.
...
const row = ([planned, conducted, used, method]: Row) =>
  ({ periodPlanned: planned, periodsConducted: conducted, novelPedagogyUsed: used, novelPedagogyMethod: method });

describe('1.1 lectureRowScore', () => {
  ...
  it('the section score is the sum of the rows, capped at 40', () => {
    const base: any = {
      cat1CourseResults: [], cat1Projects: [], cat1EContent: [], cat1ICT: [], cat2Journals: [], cat2Conferences: [],
    ...
    expect(computeScore({ ...base, cat1Courses: rows }).cat1.lectures).toBe(14); // 10 + 4 + 0
    const full = Array.from({ length: 3 }, () => row([60, 60, true, 'Flipped Classroom']));
    expect(computeScore({ ...base, cat1Courses: full }).cat1.lectures).toBe(40); // 45 capped
  });
});
