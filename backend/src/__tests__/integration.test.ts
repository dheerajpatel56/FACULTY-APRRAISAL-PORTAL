...
describe('Full appraisal workflow', () => {
  ...
  it('persists new UI-A scalar fields, coercing blank dates to null', async () => {
    if (!dbReady) return;
    ...
    const fill = await request(app)
      ...
      .send({
        categories: {
          cat1ICT: [{
            courseName: 'Data Structures', platform: 'Other', natureOfUse: 'Content Delivery',
            otherDescription: 'Custom LMS built in-house',
          }],
          cat2Journals: [{
            title: 'New Field Paper', journalName: 'Springer', authors: 'FAC21', authorPosition: 'First',
            indexed: 'SCOPUS', impactFactor: 2, impactFactorSource: 'Clarivate',
            proofFile: 'proof1.pdf', indexProofFile: 'index1.pdf',
          }],
          cat2Conferences: [{
            title: 'New Field Conf', conferenceName: 'IEEE Conf', authors: 'FAC21', authorPosition: 'First',
            presentationStatus: 'Presented',
          }],
          cat2Books: [{
            title: 'Authored Book', authors: 'FAC21', proofFile: 'bookcover.pdf',
          }],
          cat2BookChapters: [{
            title: 'A Chapter', authors: 'FAC21', authorPosition: 'First', proofFile: 'chaptercover.pdf',
          }],
          cat2Patents: [{
            title: 'New Patent', country: 'India', inventors: 'FAC21', status: 'FILED',
            iprType: 'Patent', iprTypeOther: '', dateOfFiling: '2024-01-15', dateOfPub: '',
          }],
          cat2Projects: [{
            title: 'New Project', fundingAgency: 'AICTE', amountLakhs: 5, role: 'PI', status: 'ONGOING',
            dateOfApplication: '', dateOfGrant: '2024-03-01',
          }],
          cat3IntlTravel: [{
            purpose: 'Conference travel', placeOrUniv: 'Singapore', outcome: 'Presented paper',
    ...
    expect(fill.body.cat1ICT[0].otherDescription).toBe('Custom LMS built in-house');
    expect(fill.body.cat2Journals[0].impactFactorSource).toBe('Clarivate');
    expect(fill.body.cat2Journals[0].indexProofFile).toBe('index1.pdf');
    expect(fill.body.cat2Conferences[0].presentationStatus).toBe('Presented');
    expect(fill.body.cat2Books[0].proofFile).toBe('bookcover.pdf');
    expect(fill.body.cat2BookChapters[0].proofFile).toBe('chaptercover.pdf');
    expect(fill.body.cat2Patents[0].iprType).toBe('Patent');
    expect(fill.body.cat2Patents[0].iprTypeOther).toBe(''); // non-Id/proof/date fields pass through as-is
    expect(new Date(fill.body.cat2Patents[0].dateOfFiling).toISOString().slice(0, 10)).toBe('2024-01-15');
    expect(fill.body.cat2Patents[0].dateOfPub).toBeNull(); // blank string -> null, not Invalid Date
    expect(fill.body.cat2Projects[0].dateOfApplication).toBeNull(); // blank string -> null
    expect(new Date(fill.body.cat2Projects[0].dateOfGrant).toISOString().slice(0, 10)).toBe('2024-03-01');
    expect(fill.body.cat3IntlTravel[0].fundingSource).toBe('Institute');
